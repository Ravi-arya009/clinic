...
(function ($) {
    ...
    $(".add_time_slot").on("click", function () {
        ...
        $("#checkbox_day_" + day).prop("checked", true);
    });

    //ajax header setup for csrf token
    $.ajaxSetup({
        headers: {
    ...
    $(".time-slots").on("click", ".time_slot_li", function () {
        ...
        $("#delete_slot_modal_button").attr("data-slot_id", slot_id);
    });
    ...
    //script for index pages search clinic box : Start
    $("#search").keyup(function () {
        var search_city = $("#select-city").val();
        ...
        var url =
            ...
            "&search_city=" +
            encodeURIComponent(search_city);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response["clinics"].length > 0) {
                    $("#clinics-section").show(); // Show the section if there are results
                    response["clinics"].forEach((element) => {
                        var clinicName = element["name"];
                        var clinicId = element["id"];
                        $("#clinics-results").append(
                            '<a href="/clinic/' +
                                clinicId +
                                '" class="result-link">' +
                    ...
                    $("#clinics-section").hide(); // Hide the section if there are no results
                }

                // Handle Doctors
                if (response["doctors"].length > 0) {
                    $("#doctors-section").show(); // Show the section if there are results
                    response["doctors"].forEach((element) => {
                        var doctorName = element["name"];
                        var doctorId = element["id"];
                        $("#doctors-results").append(
                            '<a href="/doctor/' +
                                doctorId +
                                '" class="result-link">' +
                                '<div class="result-item">' +
                    ...
                    $("#doctors-section").hide(); // Hide the section if there are no results
                }
            },
            error: function (xhr, status, error) {
                console.log("error");
    ...
    $("#search-results").on("mousedown", function (e) {
        e.preventDefault(); // Prevent hiding when clicking on the results
    });

    $("#search").blur(function () {
        setTimeout(function () {
            $("#search-results").css("display", "none");
        }, 200); // Slight delay to allow click events on results to trigger
    });
    //script for index pages search clinic box : End
    ...
    // doctor clinic
    if ($(".owl-carousel.our-clinics").length > 0) {
        $(".owl-carousel.our-clinics").owlCarousel({
            loop: true,
            margin: 15,
            dots: false,
            nav: true,
    ...
    $(".add-lab-test-button").on("click", function () {
        ...
        $(".add-new-lab-test").before(labTestContent);
