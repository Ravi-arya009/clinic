...
@section('content')
    <form id="update_profile" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="role" value="{{ config('role.admin') }}">
                    ...
                    <div class="clinic-logo">
                        @if (isset($admin))
                            @if (isset($admin->profile_image))
                                <img src="{{ asset('storage/profile_images/' . $admin->profile_image) }}" alt="Profile Picture">
                            @else
            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="form-wrap">
                        ...
                        <input type="phone" name="phone" id="phone" class="form-control" value="{{ old('phone', $admin->phone ?? '') }}" required>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="form-wrap">
                        <label class="col-form-label">WhatsApp</label>
                        <input type="whatsapp" name="whatsapp" id="whatsapp" class="form-control" value="{{ old('whatsapp', $admin->whatsapp ?? '') }}">
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="form-wrap">
            ...
            <div class="row">
                    ...
                    <div class="form-wrap">
                        ...
                        <select class="form-control select2_dropdown" name="state" id="state">
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" {{ old('state', $admin->state_id ?? '') == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-wrap">
                        ...
                        <select class="form-control select2_dropdown" name="city" id="city">
                            ...
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}" {{ old('city', $admin->city_id ?? '') == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-lg-6 col-md-6">
                    <div class="form-wrap">
                        ...
                        <input type="text" name="pincode" id="pincode" class="form-control" value="{{ old('pincode', $admin->pincode ?? '') }}">
                    </div>
                </div>
